...
<p><span style="color: #3498DB;">New size would be: {new_w}x{new_h} (current size: {current_w}x{current_h})</span></p>
...
def update_resize_controls(image):
    ...
    memory_per_mp = 4
    overhead = 1.0
    estimated_vram = base_memory + (mp * memory_per_mp) + overhead
    
    resize_options = calculate_resize_options(global_original_image)
    
    if not resize_options:
        resize_info = '<p style="color: #FF6347;">No resize options available. Image is at minimum allowed size.</p>'
        dropdown_update = gr.update(value=None, choices=[], interactive=False)
    else:
        resize_info = f'<p style="color: #4CAF50;">Available resize options: {", ".join(f"{opt}%" for opt in resize_options)}</p>'
        resize_info += '<p style="color: #3498DB;">These options maintain aspect ratio and meet minimum size requirements.</p>'
        dropdown_update = gr.update(value=resize_options[0], choices=resize_options, interactive=True)
    
    info_text = f"""
<p><span style="color: #FF8C00; font-weight: bold;">Original size:</span> {width}x{height}</p>
<p><span style="color: #FF8C00; font-weight: bold;">Current size:</span> {global_image.width}x{global_image.height}</p>
{resize_info}
<p><span style="color: #9932CC;">Current estimated VRAM usage: {estimated_vram:.1f}GB</span></p>
<p><span style="color: #FF6347; font-weight: bold;">Note:</span> All resizes are performed on the original image to maintain quality.</p>
<p><span style="color: #1E90FF;">Minimum allowed size: {MIN_IMAGE_SIZE}x{MIN_IMAGE_SIZE}</p>
<p><span style="color: #1E90FF;">Images are resized to multiples of {VAE_SCALE_FACTOR} pixels to meet model requirements.</span></p>
"""
...
def apply_resize(percentage):
    ...
    if percentage == 100:
        global_image = global_original_image.copy()
        return global_image, f"Image restored to original copy: {original_w}x{original_h}"
    else:
        global_image = global_original_image.resize((new_w, new_h), Image.LANCZOS)
        return global_image, f"Image resized to {new_w}x{new_h}"
